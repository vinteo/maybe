class Provider::Custom < Provider
  include ExchangeRateConcept, SecurityConcept

  # Subclass so errors caught in this provider are raised as Provider::Custom::Error
  Error = Class.new(Provider::Error)
  InvalidExchangeRateError = Class.new(Error)
  InvalidSecurityPriceError = Class.new(Error)

  def initialize(twelve_data_api_key, eodhd_api_key)
  ...
  def fetch_security_prices(symbol:, exchange_operating_mic: nil, start_date:, end_date:)
    with_provider_response do
      if exchange_operating_mic == "XASX"
        response = client.get("#{eodhd_url}/eod/#{symbol}.AU") do |req|
          req.params["api_token"] = eodhd_api_key
          req.params["fmt"] = "json"
        ...
        parsed = JSON.parse(response.body)
        result = parsed.dig("values").map do |resp|
          price = resp.dig("close")
  ...
  private
    attr_reader :twelve_data_api_key, :eodhd_api_key
    ...
    def eodhd_url
      ENV["EODHD_URL"] || "https://eodhd.com/api"
    end
    ...
    def client
      @client ||= Faraday.new(url: base_url) do |faraday|
        ...
        faraday.headers["Authorization"] = "apikey #{twelve_data_api_key}"
      end
    end
end
